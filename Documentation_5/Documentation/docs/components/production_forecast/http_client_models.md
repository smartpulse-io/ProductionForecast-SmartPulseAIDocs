...
```json
{
  ...
  "predictions": [
    ...
    {
      "deliveryStart": "2024-01-01T01:00:00+03:00",
      "value": 455.2,
      "createDate": "2024-01-01T10:30:00+03:00",
      "batchNote": "Batch A",
      "providerKey": "provider1",
...
```json
{
  "unitNo": 1,
  "period": 60,
  "firstDeliveryStart": "2024-01-01T00:00:00+03:00",
  "batchNotes": ["Batch A"],
  "providers": ["provider1"],
  "createDates": ["2024-01-01T10:30:00+03:00"],
...
The following table shows the mapping between C# property names and their JSON serialization names (using `[JsonPropertyName]` attribute):

| C# Property Name | JSON Wire Format | Class | Description |
|------------------|------------------|-------|-------------|
| `DeliveryStart` | `ds` | `Delivery` | Forecast period start time |
| `DeliveryEnd` | `de` | `Delivery`, `UnitPredictionWithDetails` | Forecast period end time |
| `UnitNo` | `uId` | `UnitPrediction` | Power plant unit identifier |

**Note**: Most properties use camelCase JSON naming policy (e.g., `DeliveryStart` → `deliveryStart`). Only specific high-frequency properties use abbreviated names to reduce payload size.

**Properties Using Standard CamelCase**:

| C# Property | JSON Name | Description |
|-------------|-----------|-------------|
| `Value` | `value` | Forecast prediction value (decimal) |
| `CreateDate` | `createDate` | When the forecast was created |
| `UpdateUser` | `updateUser` | User who updated the forecast |
| `BatchNote` | `batchNote` | Notes associated with the batch |
| `ProviderKey` | `providerKey` | Provider identifier |
| `MeasureUnit` | `measureUnit` | Unit of measure (default: 1 = MW) |
| `Index` | `index` | Normalized prediction index |
| `BatchNoteIndex` | `batchNoteIndex` | Index into BatchNotes array |
| `ProviderIndex` | `providerIndex` | Index into Providers array |
| `CreateDateIndex` | `createDateIndex` | Index into CreateDates array |
| `FirstDeliveryStart` | `firstDeliveryStart` | First delivery time in normalized forecast |
| `BatchNotes` | `batchNotes` | Array of unique batch notes |
| `Providers` | `providers` | Array of unique provider keys |
| `CreateDates` | `createDates` | Array of unique create dates |
...
**Note**: This service does NOT use Apache Pulsar or Redis.
