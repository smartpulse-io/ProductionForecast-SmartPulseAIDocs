...
### By Technology
- **IMemoryCache** → ProductionForecast uses local in-memory cache (see PROJECT_SUMMARY.md)
- **OutputCache** → ASP.NET Core 9.0 middleware (see PROJECT_SUMMARY.md)
- **EFCore SecondLevelCache** → EasyCaching InMemory provider (see docs/data/ef_core.md)
- **CDC (Change Data Capture)** → SQL Server Change Tracking (see docs/data/cdc.md)
- **SQL Server/PostgreSQL** → docs/data/ef_core.md, docs/data/schema.md
- **Entity Framework Core** → docs/data/ef_core.md
- **GraphQL** → docs/integration/graphql.md (Client only)
- **ASP.NET Core 9.0** → docs/components/production_forecast.md

### By Concern
- **Performance** → PROJECT_SUMMARY.md (metrics), docs/guides/performance.md
- **Scalability** → PROJECT_SUMMARY.md (deployment), docs/guides/deployment.md
- **Caching (ProductionForecast)** → 3-tier local cache (PROJECT_SUMMARY.md)
- **Real-time Updates** → CDC-based invalidation (docs/data/cdc.md)
- **Security** → PROJECT_SUMMARY.md (authorization flow)

### By Service/Component
- **ProductionForecast Service**
  - Architecture: PROJECT_SUMMARY.md (complete documentation)
  - API: PROJECT_SUMMARY.md (12+ endpoints)
  - Caching: 3-tier local (OutputCache + IMemoryCache + L2)
  - CDC: docs/data/cdc.md (6 trackers)
...
- **Electric.Core Library**
  ...
  - CDC (Change Data Capture): docs/data/cdc.md - **ProductionForecast uses ONLY this**
  - ❌ Pulsar: NOT used by ProductionForecast
  - ❌ Redis: NOT used by ProductionForecast

### By Audience
- **New Developer** → PROJECT_SUMMARY.md → docs/guides/setup.md
- **Architect** → PROJECT_SUMMARY.md → docs/architecture/00_system_overview.md
- **DBA** → docs/data/schema.md, docs/data/ef_core.md
- **DevOps/SRE** → docs/guides/deployment.md, PROJECT_SUMMARY.md (deployment section)
- **Performance Engineer** → docs/guides/performance.md, PROJECT_SUMMARY.md (performance section)
...
1. **Check existing**: PROJECT_SUMMARY.md (API section)
2. **Add controller**: docs/components/production_forecast/web_api_layer.md
3. **Update cache tags**: PROJECT_SUMMARY.md (DataTag format)
4. **Write tests**: docs/guides/setup.md

#### Debug performance issues
1. **Check metrics**: PROJECT_SUMMARY.md (performance characteristics)
2. **Query optimization**: docs/data/ef_core.md
3. **Cache diagnostics**: PROJECT_SUMMARY.md (cache architecture)
4. **CDC lag**: docs/data/cdc.md
5. **Tools**: docs/guides/performance.md

#### Add local cache entry (ProductionForecast)
1. **Update CacheManager**: Add new cache key using IMemoryCache
2. **Add CDC tracker**: docs/data/cdc.md
...
3. **Deploy**: docs/guides/deployment.md (Kubernetes)

#### Monitor production
1. **Metrics**: PROJECT_SUMMARY.md (throughput, latency tables)
2. **Logs**: docs/guides/troubleshooting.md
3. **Health checks**: Add /health endpoint (see Known Limitations)

#### Write unit tests
1. **Guidelines**: docs/guides/setup.md
2. **Note**: Currently no tests (Priority 0 improvement)

#### Migrate database schema
1. **EF Core migrations**: docs/data/ef_core.md
2. **Apply order**: docs/guides/setup.md
...
|----------|-------|---------|
| **Main Documentation** | PROJECT_SUMMARY.md | Complete technical documentation (1240 lines, 22 diagrams) |
| **Dependency Verification** | ACTUAL_DEPENDENCIES.md | Verified actual dependencies vs. potential dependencies |
...
## 🔗 Key Diagrams (in PROJECT_SUMMARY.md)

1. **ProductionForecast Service Architecture** - Actual implementation (local cache only)
2. **Layered Architecture** - Presentation → Application → Domain → Infrastructure → Data
3. **Request/Response Flows** - GET (cache hit), GET (cache miss), POST (save)
4. **Three-Tier Cache Architecture** - OutputCache + IMemoryCache + L2Cache (all local)
5. **CDC Architecture** - SQL Server Change Tracking integration
6. **T004ForecastLatestTracker Flow** - Output cache invalidation
7. **CacheManager Thread-Safety** - SemaphoreSlim pattern
8. **Authorization Flow** - Role-based + unit-level permissions
9. **Database Schema** - Entity relationships
10. **Deployment** - Single instance architecture
...
- **Environment Variables**: PROJECT_SUMMARY.md (configuration section)
- **Database Migrations**: docs/data/ef_core.md
...
Quick links:
- **Cache not invalidating**: docs/data/cdc.md (CDC-based invalidation)
- **Slow API response**: docs/guides/performance.md + docs/data/ef_core.md
- **Database migration failed**: docs/guides/troubleshooting.md
...
1. Read: PROJECT_SUMMARY.md - Executive Summary (5 min)
2. Read: PROJECT_SUMMARY.md - Architecture Overview (10 min)
3. Setup: docs/guides/setup.md (30 min)
4. Explore: PROJECT_SUMMARY.md - API Endpoints (10 min)

### For Architects (System design review)
1. Read: PROJECT_SUMMARY.md (20 min)
2. Review: docs/architecture/00_system_overview.md (15 min)
3. Assess: PROJECT_SUMMARY.md - Known Limitations (5 min)

### For Performance Engineers
1. Read: PROJECT_SUMMARY.md - Performance Characteristics (10 min)
2. Read: docs/guides/performance.md (20 min)
3. Review: docs/data/ef_core.md (15 min)

---

## 📞 Support

- **Documentation Issues**: Check PROJECT_SUMMARY.md first
- **Architecture Questions**: See PROJECT_SUMMARY.md - Architecture Overview
- **Component Specifics**: See PROJECT_SUMMARY.md or respective docs/components/ file
...
- **Main Document**: PROJECT_SUMMARY.md (1240 lines, 22 Mermaid diagrams)
...
See **ACTUAL_DEPENDENCIES.md** for detailed verification.
