...
**Note**: This diagram shows the complete SmartPulse microservices ecosystem. ProductionForecast uses ONLY CDC from Electric.Core, while NotificationService uses the full infrastructure stack (Pulsar, Redis, CDC).

```mermaid
%%{init: {'theme':'base', 'themeVariables': { 'fontSize':'16px', 'primaryColor':'#0288d1'}}}%%
graph TB
    subgraph Clients["🖥️ CLIENT LAYER"]
        A["Web, Mobile, Desktop Applications"]
    end

    subgraph APIs["📊 API LAYER - Microservices"]
        B["ProductionForecast Service<br/>(Port 5000)<br/>Local IMemoryCache ONLY"]
        C["NotificationService<br/>(Port 5001)<br/>Redis + Pulsar + CDC"]
    end

    subgraph Infrastructure["⚙️ ELECTRIC.CORE LIBRARY<br/>(Shared Infrastructure)"]
        direction LR
        H["CDC Trackers<br/>(SQL Change Tracking)"]
        F["Pulsar Client<br/>(Event Streaming)"]
        G["Redis Cache<br/>(Distributed Cache)"]
        I["Partitioned Queue<br/>(Message Processing)"]
    end

    subgraph External["🚀 EXTERNAL SYSTEMS"]
        direction LR
        L["Microsoft SQL Server<br/>(Primary Database)"]
        J["Apache Pulsar Cluster<br/>(Message Broker)"]
        K["Redis Cluster<br/>(Distributed Cache)"]
    end
    ...
    style F fill:#7b1fa2,stroke:#4a148c,color:#fff,stroke-width:2px
    style G fill:#7b1fa2,stroke:#4a148c,color:#fff,stroke-width:2px
    style H fill:#7b1fa2,stroke:#4a148c,color:#fff,stroke-width:2px
    style I fill:#7b1fa2,stroke:#4a148c,color:#fff,stroke-width:2px

    style J fill:#f57c00,stroke:#e65100,color:#fff,stroke-width:2px
    style K fill:#f57c00,stroke:#e65100,color:#fff,stroke-width:2px
    style L fill:#f57c00,stroke:#e65100,color:#fff,stroke-width:2px
```

**Key Points**:
- **ProductionForecast**: Uses ONLY CDC from Electric.Core for cache invalidation. All caching is local (IMemoryCache + OutputCache + L2).
- **NotificationService**: Fully distributed service using Pulsar for messaging, Redis for distributed cache, and CDC for change tracking.
- **Electric.Core**: Shared infrastructure library. Different services use different subsets of its capabilities.
- **SQL Server**: Primary database for both services.
- **Architecture Pattern**: Microservices with selective infrastructure usage based on service requirements.
